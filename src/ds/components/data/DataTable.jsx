 ...
 */
export function DataTable({ columns = [], rows = [], selectable, selected = [], onSelect,
                            onRowClick, rowKey = (r, i) => r.id ?? i, tableLayout = 'auto', style }) {
  const allOn = rows.length > 0 && selected.length === rows.length;
  const cell = { padding: 'var(--tk-row-pad-y) var(--tk-row-pad-x)', verticalAlign: 'middle' };
  return (
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout,
        ...
        <thead>
          <tr style={{ borderBottom: '1px solid var(--tk-line)' }}>
            {selectable && (
              <th style={{ ...cell, width: 44 }}>
                <Checkbox checked={allOn} indeterminate={!allOn && selected.length > 0}
                          onChange={v => onSelect?.(v ? rows.map(rowKey) : [])} />
              </th>
            )}
            {columns.map(c => (
              <th key={c.key} style={{ ...cell, width: c.width, textAlign: c.align || 'left',
                                       font: '500 12px/16px var(--tk-font-sans)',
                                       color: 'var(--tk-ink-400)', whiteSpace: 'nowrap',
        ...
        <tbody>
          {rows.map((r, i) => {
            ...
            return (
              <tr key={k} onClick={() => onRowClick?.(r)}
                  style={{ borderBottom: '1px solid var(--tk-line)',
                           background: on ? 'var(--tk-surface-cool)' : 'transparent',
