...
export function Drivers() {
  ...
  const counts = useMemo(
    () => ({
      ...
      individual: drivers.filter(
        (d) => d.registrationType === "Individual Driver",
      ).length,
      company: drivers.filter((d) => d.registrationType === "Company Driver")
        .length,
      ...
      expiringDocs: drivers.filter(
        (d) =>
          d.licenseStatus === "Expiring Soon" || d.licenseStatus === "Expired",
      ).length,
      ...
      avgRating: drivers.filter((d) => d.reviews > 0).length
        ? drivers
          ...
          drivers.filter((d) => d.reviews > 0).length
        : 0,
  ...
  const filtered = useMemo(
    () =>
      drivers.filter((d) => {
        ...
        const matchesQ =
          !q ||
          [d.name, d.license, d.phone, d.company].some((v) =>
            (v || "").toLowerCase().includes(q.toLowerCase()),
          );
        const matchesReg =
          regType === "All Types" || d.registrationType === regType;
        const matchesStatus = status === "All Status" || d.status === status;
        const matchesCompany =
          company === "All Companies" || d.company === company;
        const matchesLicense =
          licenseStatus === "All" || d.licenseStatus === licenseStatus;
        return (
          matchesTab &&
          matchesQ &&
          matchesReg &&
  ...
  function exportRoster() {
    const header =
      "Driver,Registration Type,Company,License No.,Status,License Status,Joined,Earnings,Rating\n";
    const rows = filtered
      .map((d) =>
        [
          d.name,
  ...
  const filterMenu = (open, setOpen, options, value, onChange) =>
    open && (
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button icon="user-check" onClick={() => setTab("Pending Review")}>
              Review Pending Drivers ({counts.pending})
            </Button>
            <Button variant="outline" icon="download" onClick={exportRoster}>
              Export
            </Button>
                ...
                >
                  <DropdownMenu
                    ...
                    items={[
                      ...
                      {
                        label: "Driver Compliance Report",
                        icon: "shield-check",
                        onClick: () => setMoreOpen(false),
                      },
                    ]}
      ...
      >
        <StatCard
          ...
          label="Total Drivers"
          value={counts.total}
          caption="All registered drivers"
        />
        <StatCard
          icon="user"
          tint="green"
      ...
      >
        ...
        <StatCard
          ...
          value={naira(counts.totalEarnings)}
        ...
        <StatCard
          ...
          value={`${counts.avgRating.toFixed(1)} / 5`}
      ...
      >
        <Card pad="none">
          <TableToolbar
            search={
              <SearchField
                placeholder="Search by driver name, license number, phone, or company..."
                value={q}
            ...
            filters={
              <>
                <span style={{ position: "relative" }}>
                  <FilterSelect
                    label={regType}
                    active={regType !== "All Types"}
                    onClick={() =>
                      setOpenMenu(openMenu === "reg" ? null : "reg")
                    }
                  />
                  {filterMenu(
                    openMenu === "reg",
                ...
                <span style={{ position: "relative" }}>
                  <FilterSelect
                    ...
                    active={licenseStatus !== "All"}
                    onClick={() =>
                      setOpenMenu(openMenu === "license" ? null : "license")
                    }
                  />
                  {filterMenu(
                    openMenu === "license",
                    setOpenMenu,
                    ["All", ...LICENSE_STATUSES],
                    licenseStatus,
                    setLicenseStatus,
                  )}
                </span>
            ...
            columns={[
              { key: "name", header: "Driver", render: driverCell },
              {
                ...
                render: (r) => (
                  <Badge
                    tone={
                      r.registrationType === "Company Driver"
                        ? "info"
                        : "neutral"
                    }
                  >
              ...
              },
              {
                key: "license",
                header: "License No.",
                render: (r) => <span className="tk-mono">{r.license}</span>,
              },
              {
                key: "truckPlate",
                ...
                render: (r) =>
                  r.truckPlate ? (
                    <span className="tk-mono">{r.truckPlate}</span>
                  ) : (
                    <span className="tk-meta">Not Assigned</span>
                  ),
              },
              {
                key: "status",
              ...
              },
              {
                ...
                render: (r) => <Badge>{r.licenseStatus}</Badge>,
              },
                          ...
                          <DropdownMenu
                            ...
                            items={
                              r.status === "Pending Review"
                                ...
                                : STATUSES.filter(
                                    (s) =>
                                      s !== r.status && s !== "Pending Review",
                                  ).map((s) => ({
                                    label: `Mark as ${s}`,
                                    icon: "check",
                                    onClick: () => {
                                      setDriverStatus(r.id, s);
                                      setRowMenu(null);
                                    },
                                  }))
                            }
        ...
        >
          ...
          <SectionCard title="Earnings Overview" description="Last 6 months">
            ...
            <div
              style={{
                marginTop: 6,
                font: "700 20px/28px var(--tk-font-sans)",
            ...
            >
              <Icon name="arrow-up" size={12} color="var(--tk-success)" /> 18%
          ...
          <SectionCard
            ...
            action={
              <a onClick={() => setTab("Active")} style={{ cursor: "pointer" }}>
                View all
              </a>
            }
          >
            <div style={{ display: "grid", gap: 12 }}>
              {topRated.map((d, i) => (
                ...
                >
                  <span className="tk-meta" style={{ width: 14 }}>
                    {i + 1}
                  </span>
                  ...
                  >
                    <Icon name="star" size={13} color="var(--tk-warning)" />
                    {d.rating.toFixed(1)}{" "}
                    <span className="tk-meta">({d.reviews})</span>
                  </span>
                </Link>
              ))}
            </div>
          ...
          <QuickActionsCard
            items={[
              {
                icon: "user-check",
                label: "Review Pending Drivers",
                hint: "Verify and approve driver registrations",
                onClick: () => setTab("Pending Review"),
              },
              {
                icon: "shield-check",
                label: "Verify Documents",
                hint: "Review licenses and other documents",
              },
              {
                icon: "file-chart-column",
                label: "Driver Compliance Report",
                hint: "View compliance and expiry report",
              },
              {
                icon: "download",
                label: "Export Driver List",
                hint: "Download driver data report",
