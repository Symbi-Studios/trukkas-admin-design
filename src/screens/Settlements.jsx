...
import {
  ...
  settlementAlerts,
} from "../mock/fixtures/settlements.js";
...
const TYPE_TONE = {
  "Driver Payout": "blue",
  "Transporter Payment": "purple",
  "Vendor Payment": "teal",
  "Customer Refund": "orange",
};
...
export function Settlements() {
  ...
  const tabFiltered = useMemo(() => {
    if (tab === "Driver Settlements")
      return all.filter((s) => s.type === "Driver Payout");
    if (tab === "Vendor Settlements")
      return all.filter((s) => s.type === "Vendor Payment");
    if (tab === "Customer Settlements")
      return all.filter(
        (s) => s.type === "Customer Refund" || s.payeeType === "Customer",
      );
    return all;
  }, [all, tab]);

  const filtered = useMemo(() => {
    let rows = tabFiltered.filter(
      (s) =>
        ...
        (!q ||
          [s.id, s.relatedTo, s.payer, s.payee].some(
  ...
  return (
    <>
      <PageHeader
        crumbs={["Business & Finance", "Settlements"]}
        title="Settlements"
      ...
      >
        <StatCard
          ...
          value={formatNaira(settlementSummary.totalSettlements)}
          delta={settlementSummary.totalSettlementsDelta}
        ...
        <StatCard
          ...
          value={formatNaira(settlementSummary.completedSettlements)}
          delta={settlementSummary.completedSettlementsDelta}
          caption="vs May 17 – May 23"
        />
        <StatCard
          icon="hourglass"
          ...
          value={formatNaira(settlementSummary.pendingSettlements)}
          delta={settlementSummary.pendingSettlementsDelta}
          direction="down"
          caption="vs May 17 – May 23"
        />
        <StatCard
          ...
          tint="purple"
          label="On Hold"
          value={formatNaira(settlementSummary.onHold)}
          delta={settlementSummary.onHoldDelta}
          direction="down"
          caption="vs May 17 – May 23"
        />
        <StatCard
          ...
          value={formatNaira(settlementSummary.failedSettlements)}
          delta={settlementSummary.failedSettlementsDelta}
          direction="down"
          caption="vs May 17 – May 23"
        />
        <StatCard
          ...
          value={settlementSummary.avgSettlementTime}
          delta={settlementSummary.avgSettlementTimeDelta}
          caption="vs May 17 – May 23"
        />
      </div>
      ...
      >
            ...
            filters={
              <>
                <FilterButton
                  ...
                  active={settlementType !== "All Types"}
                  onChange={(v) => {
                    setSettlementType(v);
                    setPage(1);
                  }}
                ...
                <FilterButton
                  label="Payer Type"
                  value={payerType}
                  options={["All Payers", "Importer", "Customer", "Internal"]}
                  active={payerType !== "All Payers"}
                  onChange={(v) => {
                    setPayerType(v);
                    setPage(1);
                  }}
                ...
                <FilterButton
                  ...
                  value={method}
                  options={["All Methods", "Bank Transfer", "Wallet"]}
                  active={method !== "All Methods"}
                  onChange={(v) => {
                    setMethod(v);
                    setPage(1);
                  }}
                ...
                columns={[
                  {
                    ...
                    render: (r) => (
                      <span style={{ display: "grid", gap: 2 }}>
                        <Link to={`/settlements/${r.id}`} className="tk-mono">
                          {r.id}
                        </Link>
                      </span>
                    ),
                  ...
                  },
                  {
                    ...
                    render: (r) => (
                      <span style={{ display: "grid", gap: 2 }}>
                        <span>{r.payer}</span>
                        <span className="tk-meta">{r.payerType}</span>
                      </span>
                    ),
                  },
                  {
                    ...
                    render: (r) => (
                      <span style={{ display: "grid", gap: 2 }}>
                        <span>{r.payee}</span>
                        <span className="tk-meta">{r.payeeType}</span>
                      </span>
                    ),
                  },
                  {
                  ...
                  },
                  {
                    ...
                    render: (r) =>
                      r.settledOn ? (
                        <span style={{ display: "grid", gap: 2 }}>
                          <span>{r.settledOn}</span>
                          <span className="tk-meta">{r.settledTime}</span>
                        </span>
                      ) : (
                        "—"
                      ),
        ...
        >
          <SectionCard
            ...
            footer={<a href="#">View Report →</a>}
          >
            <div style={{ display: "grid", justifyItems: "center", gap: 14 }}>
              <DonutChart
                size={160}
                thickness={18}
                centerValue={`₦${(settlementSummary.totalSettlements / 1000000).toFixed(2)}M`}
                centerLabel="Total Settled"
                data={payoutSummary.map((d) => ({
                  label: d.label,
                  value: d.pct,
                  color: d.color,
                }))}
          ...
          >
            {settlementStatusBreakdown.map((s) => (
              ...
              >
                ...
                >
                  {s.count.toLocaleString("en-NG")} settlements
                </span>
