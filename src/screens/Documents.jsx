...
const TYPE_LABEL = {
  TDO: "TDO",
  "Exit Note": "Exit Note",
  "Indemnity Letter": "Indemnity Letter",
  Custom: "Custom Documents",
  Exporter: "Exporter Documents",
  Other: "Other Documents",
};
const TYPE_TONE = {
  TDO: "blue",
  "Exit Note": "orange",
  "Indemnity Letter": "purple",
  Custom: "teal",
  Exporter: "success",
  Other: "neutral",
};
...
const INITIAL_DOCS = [
  {
    id: "d1",
    name: "TDO (Truck Dispatch Order)",
    desc: "Dispatch order for truck movement",
    type: "TDO",
    route: "Apapa Port → Ikeja Warehouse",
    job: "JOB-29821",
    trip: "TRIP-98231",
    createdBy: "John Adewale",
    dept: "Operations",
    status: "Verified",
    date: "May 30, 2026",
    time: "09:42 AM",
  },
  {
    id: "d2",
    name: "Exit Note",
    desc: "Gate exit clearance note",
    type: "Exit Note",
    route: "Lagos Port → Multiple Sites (20)",
    job: "JOB-29814",
    trip: "TRIP-98214",
    createdBy: "S. Musa",
    dept: "Documentation",
  ...
  {
    id: "d3",
    name: "Custom Clearance Document",
    desc: "Customs clearance certificate",
    type: "Custom",
    route: "Tin Can Port → Victoria Island",
    job: "JOB-29820",
    trip: "TRIP-98230",
    createdBy: "Victoria Udo",
    dept: "Finance Team",
    status: "Verified",
    date: "May 29, 2026",
    time: "06:30 PM",
  },
  {
    ...
    desc: "Indemnity letter for cargo release",
    type: "Indemnity Letter",
    route: "Apapa Port → Onne Port",
    job: "JOB-29819",
    trip: "TRIP-98219",
    createdBy: "A. Ibrahim",
    dept: "Documentation",
  ...
  {
    id: "d5",
    name: "Exporter Document",
    desc: "Exporter registration & details",
    type: "Exporter",
  ...
  },
  {
    id: "d6",
    name: "Bill of Lading",
    desc: "Copy of bill of lading",
    type: "Other",
    route: "Lekki Warehouse → Apapa Port",
    job: "JOB-29822",
    trip: "TRIP-98222",
    createdBy: "John Adewale",
    dept: "Operations",
  ...
  },
  {
    ...
    type: "Other",
    route: "Tin Can Island → Lagos Island",
    job: "JOB-29816",
    trip: "TRIP-98216",
    createdBy: "S. Musa",
    dept: "Finance Team",
  ...
  },
  {
    id: "d8",
    name: "Packing List",
    desc: "Cargo packing list",
    type: "Other",
    route: "Apapa Port → Lekki Warehouse",
    job: "JOB-29817",
    trip: "TRIP-98217",
    createdBy: "Victoria Udo",
    dept: "Documentation",
  ...
  },
  {
    ...
    desc: "Cargo insurance certificate",
    type: "Other",
    route: "PH Port → Aba Enugu Road",
    job: "JOB-29812",
    trip: "TRIP-98212",
    createdBy: "A. Ibrahim",
    dept: "Documentation",
...
const ALERTS = [
  {
    icon: "triangle-alert",
    tone: "amber",
    title: "78 documents will expire within 30 days",
    desc: "Review and update to avoid disruption",
    time: "Just now",
  },
  {
    icon: "triangle-alert",
  ...
  },
  {
    icon: "circle-alert",
    tone: "red",
    title: "3 documents were rejected",
    desc: "Click to review and re-upload",
    time: "1h ago",
  },
];
...
export function Documents() {
  ...
  useEffect(() => {
    setPage(1);
  }, [tab, q, typeFilter, jobFilter, statusFilter, userFilter]);

  const jobs = useMemo(() => [...new Set(docs.map((d) => d.job))], [docs]);
  const users = useMemo(
    () => [...new Set(docs.map((d) => d.createdBy))],
    [docs],
  );
      ...
      docs.filter(
        (d) =>
          ...
          (dateRange === "Last 30 Days" ||
            (dateRange === "May 17 – May 23, 2026"
              ? /May (1[7-9]|2[0-3]), 2026/.test(d.date)
              : /May (2[4-9]|30), 2026/.test(d.date))) &&
          (!q ||
            [d.name, d.job, d.trip, d.route].some((v) =>
  ...
  function downloadDocument(d) {
    ...
    anchor.href = url;
    anchor.download = d.name.replaceAll(" ", "_") + ".txt";
    anchor.click();
    URL.revokeObjectURL(url);
    notify("info", `${d.name} downloaded.`);
  }
    ...
    notify(
      status === "Verified"
        ...
        : status === "Rejected"
          ? "danger"
          : "info",
      `${status === "Rejected" ? "Marked" : "Document"} ${status === "Verified" ? "verified" : status === "Rejected" ? "rejected" : "set to " + status}.`,
    );
  }
  ...
  function bulkVerify() {
    bulkUpdateDocuments(selected, {
      status: "Verified",
      compliance: "Compliant",
    });
    notify(
      "success",
      `${selected.length} document${selected.length === 1 ? "" : "s"} verified.`,
    );
    setSelected([]);
  }
  function bulkDelete() {
    ...
    notify(
      "warning",
      `${selected.length} document${selected.length === 1 ? "" : "s"} deleted.`,
    );
    setSelected([]);
  }
  ...
  const columns = [
        ...
        >
          <span
            style={{
              ...
              color:
                TYPE_TONE[d.type] === "neutral"
                  ? "var(--tk-ink-500)"
                  : `var(--tk-${TYPE_TONE[d.type]})`,
            }}
          >
    ...
    },
    {
      ...
      width: 110,
      render: (d) => <Tag tone={TYPE_TONE[d.type]}>{d.type}</Tag>,
    },
    {
      key: "related",
      header: "Related To",
  ...
  return (
    <>
      <PageHeader
        title="Documents"
        description="Manage, generate and track all documents across Trukkas."
        actions={
          <>
                ...
                >
                  <DropdownMenu
                    ...
                    items={[
                      ...
                      {
                        label: "Pending Verification Only",
                        icon: "clock",
                        onClick: () => {
                          setStatusFilter("Pending");
                          setOpenFilter(null);
      ...
      <div className="documents-main">
        <SectionCard pad="none">
          <div className="documents-filters">
            <SearchField
              placeholder="Search documents by name, job ID, trip ID..."
            ...
            <span style={{ position: "relative" }}>
              <FilterSelect
                label={jobFilter === "All" ? "Related To" : jobFilter}
                active={jobFilter !== "All"}
                onClick={() =>
                  setOpenFilter(openFilter === "job" ? null : "job")
                }
              />
              {openFilter === "job" && (
                <span
                ...
                >
                  <DropdownMenu
                    width={190}
                    items={["All", ...jobs].map((s) => ({
                      label: s,
                      icon: s === jobFilter ? "check" : undefined,
                      onClick: () => {
                        setJobFilter(s);
                        setOpenFilter(null);
                      },
            ...
            <span style={{ position: "relative" }}>
              <FilterSelect
                label={userFilter}
                active={userFilter !== "All Users"}
                onClick={() =>
                  setOpenFilter(openFilter === "user" ? null : "user")
                }
              />
              {openFilter === "user" && (
                <span
                ...
                >
                  <DropdownMenu
                    width={190}
                    items={["All Users", ...users].map((s) => ({
                      label: s,
                      icon: s === userFilter ? "check" : undefined,
                      onClick: () => {
                        setUserFilter(s);
                        setOpenFilter(null);
                      },
        ...
        <div className="documents-rail">
          ...
          >
            {CATEGORIES.map((c) => (
              <div
                key={c.type}
                className="documents-cat-row"
                onClick={() => {
                  setTab(
                    TAB_ITEMS.find((t) => TAB_TYPE[t] === c.type) ||
                      "All Documents",
                  );
                }}
              >
                <span
                  style={{
                    ...
                    color:
                      TYPE_TONE[c.type] === "neutral"
                        ? "var(--tk-ink-500)"
                        : `var(--tk-${TYPE_TONE[c.type]})`,
                  }}
                >
          ...
          <SectionCard
            ...
            action={
              <a
                onClick={() => notify("info", "Showing recent documents.")}
                style={{ cursor: "pointer" }}
              >
                View All
              </a>
          ...
          >
            {RECENT.map((r) => (
              <ListRow
                key={r.name}
                icon="file-text"
                iconTint={LIST_TINT[TYPE_TONE[r.type]]}
                title={r.name}
                subtitle={`${r.job} · ${r.trip}`}
          ...
          <SectionCard
            ...
            action={
              <a
                onClick={() => notify("info", "Showing all document alerts.")}
                style={{ cursor: "pointer" }}
              >
                View All
              </a>
          ...
          >
            {ALERTS.map((a) => (
              <AlertRow
                ...
                description={a.desc}
                action={
                  <span className="tk-meta" style={{ whiteSpace: "nowrap" }}>
                    {a.time}
                  </span>
                }
              />
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button disabled={!form.name || !form.job} onClick={handleCreate}>
              Upload Document
            </Button>
          </>
        }
        ...
        <div style={{ display: "grid", gap: 14 }}>
          ...
          >
            <TextField
              label="Job ID"
              required
              placeholder="e.g. JOB-29821"
              value={form.job}
              onChange={(e) => setForm((f) => ({ ...f, job: e.target.value }))}
            />
            <TextField
              label="Trip ID"
              placeholder="e.g. TRIP-98231"
              value={form.trip}
